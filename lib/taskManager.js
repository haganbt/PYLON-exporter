...
var buildFromResponse = function buildFromResponse(data, task) {
    ...
    for (var ind in results) {
        if (results.hasOwnProperty(ind)) {
            ...
            var reqObj = _getDefaultTaskObj() //new req params obj
                , configFilter = ""
                ;

            //name property
            reqObj.name = task.name;
            ...
            for (var thenInd in then) {
                if (then.hasOwnProperty(thenInd)) {
                    ...
                    reqObj.json.parameters.parameters[thenInd]
                            = then[thenInd];
                }
            }
            ...
            if(resultTarget){
                reqObj.json.filter = configFilter + resultTarget + " "
                ...
                if(startOverride > moment.utc().subtract(32, 'days').unix()){
                    reqObj.json.start = startOverride;
                }
                reqObj.json.end = endOverride;

                log.debug("Start: " + reqObj.json.start +
                    " " + moment.unix(reqObj.json.start).utc().format("DD-MMM h:mm:ss a"));
                log.debug("End:   " + reqObj.json.end
                    + " " + moment.unix(reqObj.json.end).utc().format("DD-MMM h:mm:ss a"));
            }
            ...
            //log.info(JSON.stringify(reqObj, null, 4));
            secondaryTasks.push(reqObj);
        }
    }
