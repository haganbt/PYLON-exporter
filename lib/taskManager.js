...
var _addTaskParams = function _addTaskParams(reqParams, tasks) {
    for (var taskProp in tasks) {
        if (tasks.hasOwnProperty(taskProp)) {
            ...
            if (taskProp === 'then') {
                ...
                //write the name prop to the root
                if(taskProp === "name"){
                    reqParams.name = tasks[taskProp];
                } else {
...
var _processTask = function _processTask(taskType, eachTask){
    ...
    //iterate the task properties
    for (var property in eachTask) {
        if (eachTask.hasOwnProperty(property)) {
...
var _processMergedTask = function _processMergedTask(taskType, name, mergeObject){
    ...
    for (var mergeTasks in mergeObject[name]) {
        if (mergeObject[name].hasOwnProperty(mergeTasks)) {
            ...
            // write the name to each task to be consistent
            // with non-merged tasks
            eachTask.name = name || "no_name_specified";
