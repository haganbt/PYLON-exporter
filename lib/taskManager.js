...
var buildFromConfig = function buildFromConfig() {
    ...
    log.debug("Building requests from config...");
    ...
    log.debug("Completed building requets from config.");
    return allTasks;
};
...
var buildFromResponse = function buildFromResponse(data, then) {
    ...
    for (var ind in results) {
        if (results.hasOwnProperty(ind)) {
            ...
            //reqParams.mergeCache[mergeId].taskCount = results.length;
            //reqParams.mergeCache[mergeId].key = [results[ind].key];
