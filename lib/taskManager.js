 ...
 * @private
 */
var _getMergeKey = function _getMergeKey(key) {
     if(key === undefined){
                    ...
                    if(isArray(tasks[analysisType][taskKey]) === true){
                        ...
                        for (var mergeTasks in tasks[analysisType][taskKey]) {
                            if (tasks[analysisType][taskKey].hasOwnProperty(mergeTasks)) {
                                ...
                                //cache the merge key - target for fd and filter for ts
                                mergeCache.add(cacheId,
                                    _getMergeKey(tasks[analysisType][taskKey][mergeTasks].target ||
                                        reqParams.json.filter));

                                //attach a signature to the request - target for fd and filter for ts
                                reqParams.cache = {
                                    "cacheId": cacheId,
...
var buildFromResponse = function buildFromResponse(data, then) {
    ...
    for (var ind in results) {
        if (results.hasOwnProperty(ind)) {
            ...
            //cache the merge key
            mergeCache.add(cacheId, _getMergeKey(results[ind].key));
