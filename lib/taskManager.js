                ...
                if (tasks[analysisType].hasOwnProperty(taskKey)) {
                    ...
                    if(isArray(tasks[analysisType][taskKey]) === true){
                        ...
                        for (var mergeTasks in tasks[analysisType][taskKey]) {
                            ...
                            //[3] task params
                            reqParams = _addTaskParams(reqParams, tasks[analysisType][taskKey][mergeTasks]);


                            /*
                            for (var paramKey in tasks[analysisType][taskKey][mergeTasks]) {
                                if (tasks[analysisType][taskKey][mergeTasks]
                                        .hasOwnProperty(paramKey)) {

                                    //copy the 'then' object to params
                                    if (paramKey === 'then') {
                                        reqParams.then =
                                            tasks[analysisType][taskKey][mergeTasks].then;
                                    } else {
                                        reqParams.json.parameters.parameters[paramKey]
                                            = tasks[analysisType][taskKey][mergeTasks][paramKey];
                                    }
                                }
                            }
                            */
